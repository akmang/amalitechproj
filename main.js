const jobsData= [
    ...
    },
    {
      ...
      "logo": "icons/coffeeroastersicon.png",
    ...
    },
    {
      "id": 10,
      "company": "Mastercraft",
      "logo": "icons/mastercrafticon.png",
  ...
  const fullTimeCheckbox = document.getElementById('checkerbox');
  ...
  displayJobs(initialJobCount);
  
  function displayJobs(count) {
    setCounts();
    const filteredJobs = filterJobsData();
    const jobChoice = document.getElementById('jobchoice');
    jobChoice.innerHTML = filteredJobs
      .slice(0, count)
      .map(jobTemplate)
      .join('');
  
    // Hide the load more button if all jobs have been displayed
    const loadMoreButton = document.getElementById('forMore');
    if (filteredJobs.length <= count) {
  ...
  function loadMoreJobs() {
    const jobChoice = document.getElementById('jobchoice');
    const displayedJobsCount = jobChoice.getElementsByClassName('jobencasing').length;
    const filteredJobs = filterJobsData();
    const remainingJobs = filteredJobs.slice(
      displayedJobsCount,
      displayedJobsCount + loadMoreIncrement
    );
  
    if (remainingJobs.length > 0) {
      jobChoice.innerHTML += remainingJobs.map(jobTemplate).join('');
    }
  
    // Hide the load more button if all jobs have been displayed
    const loadMoreButton = document.getElementById('forMore');
    if (displayedJobsCount + remainingJobs.length === filteredJobs.length) {
      loadMoreButton.style.display = 'none';
    }
  }
  ...
  function filterJobsData() {
    const searchText = jobSearchInput.value.toLowerCase();
    const locationText = locationSearchInput.value.toLowerCase();
    const isFullTime = fullTimeCheckbox.checked;
  
    return jobsData.filter((job) => {
      const titleMatch = job.position.toLowerCase().includes(searchText);
      const companyMatch = job.company.toLowerCase().includes(searchText);
      const locationMatch = job.location.toLowerCase().includes(locationText);
      const isFullTimeMatch = !isFullTime || job.contract.toLowerCase() === 'full time';
  
      return (titleMatch || companyMatch || locationMatch) && isFullTimeMatch;
    });
  }
  
  function jobTemplate(job) {
    return `
      <div class="jobencasing" id="jobencasing">
        <div class="job">
          ...
          <br>
          <h7 class="details">${job.company}</h7>
          <br>
  ...
  // Call the loadMoreJobs function initially to check if the load more button should be displayed
  loadMoreJobs();
  
  // Reload jobs on window resize to adjust the number of jobs per row
...
//     const jobHTML = jobsToDisplay.map(jobTemplate).join('');
//     const jobChoiceElement = document.getElementById("jobchoice");
//     jobChoiceElement.innerHTML += jobHTML;

//     // Hide "Load More" button if all jobs are displayed
//     if (endIndex >= jobs.length) {
//         document.getElementById("forMore").style.display = "none";
//     }
// }
...
// document.getElementById("forMore").addEventListener("click", loadMoreJobs);